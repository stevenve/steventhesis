...
void CBTFootbotExp2Controller::Init ( TConfigurationNode& t_tree)
{
	...
	m_pcState->Init();
	m_pcRootBehavior = new CBTFootbotExp2RootBehavior(m_pcRobotData);
...
void CBTFootbotExp2Controller::ControlStep (  )
{
	// Call the control step in of the interface
	CCI_BehaviorController<CCI_FootBotState>::ControlStep();
}

UInt32 CBTFootbotExp2Controller::GetRootBehaviorStateID()
{
	CBTFootbotExp2RootBehavior* pcRootBehavior = (CBTFootbotExp2RootBehavior*) m_pcRootBehavior;
	return pcRootBehavior->GetStateID();
}

std::string CBTFootbotExp2Controller::GetRootBehaviorStateName()
{
	CBTFootbotExp2RootBehavior* pcRootBehavior = (CBTFootbotExp2RootBehavior*) m_pcRootBehavior;
	return pcRootBehavior->GetStateName();
}

void CBTFootbotExp2Controller::Destroy (  )
{
 ...
 */
REGISTER_CONTROLLER( CBTFootbotExp2Controller, "footbot_exp2_controller" );
