...
CBTFootbotCombinedRootBehavior::CBTFootbotCombinedRootBehavior(CCI_RobotData<CCI_FootBotState>* c_robot_data, TConfigurationNode& t_node) :
...
CBTFootbotCombinedRootBehavior::~CBTFootbotCombinedRootBehavior() {

}

/****************************************/

void CBTFootbotCombinedRootBehavior::Init(CCI_FootBotState& state) {
	c_robot_state = &state;
...
void CBTFootbotCombinedRootBehavior::Destroy(CCI_FootBotState& c_robot_state) {
...
void CBTFootbotCombinedRootBehavior::Reset(CCI_FootBotState& c_robot_state) {
...
void CBTFootbotCombinedRootBehavior::StepSolitary(CCI_FootBotState& c_robot_state) {
...
void CBTFootbotCombinedRootBehavior::StepRecruitee(CCI_FootBotState& c_robot_state) {
...
void CBTFootbotCombinedRootBehavior::StepRecruiter(CCI_FootBotState& c_robot_state) {
...
void CBTFootbotCombinedRootBehavior::SignalPickUp(){
...
void CBTFootbotCombinedRootBehavior::FollowSignal(){
...
void CBTFootbotCombinedRootBehavior::ExploreSignalArea(){
	--signalExploreTimer;
...
void CBTFootbotCombinedRootBehavior::PickUp(){
	CVector2 vect = CVector2(0,0);
	m_pcMotionControl->ComputeSpeedFromForce(vect);
	m_pcMotionControl->Step(*c_robot_state);
...
void CBTFootbotCombinedRootBehavior::Drop(){
	CVector2 vect = CVector2(0,0);
	m_pcMotionControl->ComputeSpeedFromForce(vect);
	m_pcMotionControl->Step(*c_robot_state);
...
void CBTFootbotCombinedRootBehavior::Explore() {
...
void CBTFootbotCombinedRootBehavior::ExitNest() {
...
void CBTFootbotCombinedRootBehavior::GoToFood(){
...
void CBTFootbotCombinedRootBehavior::ReturnToNest() {
...
void CBTFootbotCombinedRootBehavior::StartOdometry(){
	m_pcOdometry->Reset(*c_robot_state);
	m_pcOdometry->Start();
}

void CBTFootbotCombinedRootBehavior::StopOdometry(){
	m_pcOdometry->Stop();
}

bool CBTFootbotCombinedRootBehavior::InNest(){
...
void CBTFootbotCombinedRootBehavior::GoToVector(CVector2 vec){
	m_pcMotionControl->ComputeSpeedFromForce(vec);
	m_pcMotionControl->Step(*c_robot_state);
}

bool CBTFootbotCombinedRootBehavior::IsDoneLookingForFood(){
	CVector2 tmp = m_pcOdometry->GetReversedLocationVector();
	return tmp.Length() <= 7;
}

void CBTFootbotCombinedRootBehavior::ResetOdometry(){
	m_pcOdometry->Reset(*c_robot_state);
}
...
void CBTFootbotCombinedRootBehavior::SFoodData::Reset() {
	HasFoodItem = false;
	FoodItemIdx = 0;
	TotalFoodItems = 0;
	FoodPatchIdx = 0;
...
void CBTFootbotCombinedRootBehavior::UpdateFoodData(){
	//TODO statistics
}

/******/

CBTFootbotCombinedRootBehavior::SStateData::SStateData() : CurrentPosition(0,0){
	State = STATE_EXPLORING;
	InNest = true;
}

void CBTFootbotCombinedRootBehavior::SStateData::Init(/*TConfigurationNode& t_node*/) {
	Reset();
}

void CBTFootbotCombinedRootBehavior::SStateData::Reset() {
	State = STATE_EXPLORING;
	InNest = false;
}

void CBTFootbotCombinedRootBehavior::UpdateStateDataSolitary(){
	if(InNest())
...
void CBTFootbotCombinedRootBehavior::UpdateStateDataRecruitee(){
	m_pcSignalling->Step(*c_robot_state);
...
void CBTFootbotCombinedRootBehavior::UpdateStateDataRecruiter(){
	if(InNest())
		m_sStateData.InNest = true;
	else
		m_sStateData.InNest = false;
