...
public:
	...
	CCI_SwarmanoidRobotState& operator=(const CCI_SwarmanoidRobotState& c_state) {
		...
		if (this != &c_state) {
			...
			m_pcSteeringSensor     = c_state.m_pcSteeringSensor;

			/** Copy the sensors booleans */
			m_bIsUsingRABSensor     = c_state.m_bIsUsingRABSensor;
			m_bIsUsingSteeringSensor = c_state.m_bIsUsingSteeringSensor;
	...
	virtual inline const CCI_DifferentialSteeringSensor::SReading GetSteeringReading(){
		CHECK_IS_SENSOR_USED_HELPER(STEERING_SENSOR_XML_NAME,m_bIsUsingSteeringSensor,"GetSteeringReading");
		return m_pcSteeringSensor->GetReading();
	}
	...
	virtual inline void ClearRABPacketData() {
		...
		CByteArray clear = CByteArray(10,0);
		::memcpy(&m_tRABPacketDataToSend, &clear, sizeof(CByteArray));
		//::memset(&m_tRABPacketDataToSend, 0, sizeof(CByteArray));
	}
...
protected:
	...
	CCI_DifferentialSteeringSensor* m_pcSteeringSensor;

	/** Sensors booleans */
	bool m_bIsUsingRABSensor;
	bool m_bIsUsingSteeringSensor;

	/* XML sensor names */
	static const std::string RAB_SENSOR_XML_NAME;
	static const std::string STEERING_SENSOR_XML_NAME;
