...
/*dynamic_cast<CPP_TYPE*>(m_pcRobot->GetSensor(STRING_TYPE)); */
#define SENSOR_INIT_HELPER(STRING_TYPE, CPP_TYPE, MEMBER_VAR, USING_VAR)        \
   ...
   if (itSensors != mapSensors.end()) {                                         \
      MEMBER_VAR = (m_pcController->GetSensor<CPP_TYPE>(STRING_TYPE));  \
      USING_VAR = true;                                                         \
   }
...
#define ACTUATOR_INIT_HELPER(STRING_TYPE, CPP_TYPE, MEMBER_VAR, USING_VAR)      \
   ...
   if (itActuators != mapActuators.end()) {                                     \
      MEMBER_VAR = (m_pcController->GetActuator<CPP_TYPE>(STRING_TYPE));\
      USING_VAR = true;                                                         \
   }
