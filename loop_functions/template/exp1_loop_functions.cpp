...
void CExp1LoopFunctions::Init(TConfigurationNode& t_node) {
   try {
	   arenaSize = CVector2(GetSpace().GetArenaSize().GetX(),GetSpace().GetArenaSize().GetY());
...
bool CExp1LoopFunctions::InNest(const CVector2& pos){
    if(pos.GetX() > -(nestSize/2) && pos.GetX() < (nestSize/2) && pos.GetY() > -(nestSize/2) && pos.GetY() < (nestSize/2)) {
      return true;
   }
   return false;
}
